...
class IA:
    ...
    def __init__(self, mapa):
        ...
        self.celdas_camino = []
        self.coordenada_actual = mapa.celda_origen
    ...
    def avanzar(self):
        ...
        if len(self.celdas_camino) == 0:
            self.celdas_camino.append(self.coordenada_actual)

        celda = self.celdas_camino[-1]
        self.celdas_visitadas.add(celda)

        if celda == self.mapa.destino():
            return

        celdas_vecinas = coordenadas_vecinas(celda, self.mapa)
        for vecina in celdas_vecinas:
            if not vecina in self.celdas_visitadas and not self.mapa.celda_bloqueada(vecina):
                self.celdas_camino.append(vecina)
                self.coordenada_actual = self.celdas_camino[-1]
                return

        self.celdas_camino.pop()
